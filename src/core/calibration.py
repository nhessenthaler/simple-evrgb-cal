...
def find_chessboard_corners(gray_image: np.ndarray, pattern_size: tuple[int, int] = (9, 6)) -> tuple[bool, np.ndarray]:
    """
    Function that finds the corners of a chessboard pattern in an image.

    Args:
        gray_image (np.ndarray): The input grayscale image in which to find the chessboard corners.
        pattern_size (tuple [int, int]): The size of the chessboard pattern (number of inner corners per row and column).

    Returns:
        succes (bool): A boolean indicating whether the corners were found.
        corners (np.ndarray): An array of the corner positions.
    """

    # Find the chessboard corners using OpenCV implementation
    t_success, t_corners = cv2.findChessboardCorners(gray_image, pattern_size, None)

    return t_success, t_corners


def refine_chessboard_corners(gray_image: np.ndarray, corners: np.ndarray) -> np.ndarray:
    """
    Function that refines the corner positions of a chessboard pattern in an image. Acheives subpixel accuracy for the corner positions.

    Args:
        gray_image (np.ndarray): The input grayscale image in which to refine the chessboard corners.
        corners (np.ndarray): The initial corner positions to be refined.

    Returns:
        refined_corners (np.ndarray): An array of the refined corner positions.
    """

    # Find the subpixel accurate corner positions using OpenCV implementation
    t_criteria = (cv2.TERM_CRITERIA_EPS + cv2.TERM_CRITERIA_MAX_ITER, 30, 0.001)
    t_refined_corners = cv2.cornerSubPix(gray_image, corners, (11, 11), (-1, -1), t_criteria)

    return t_refined_corners


def draw_chessboard_corners(
    image: np.ndarray, pattern_size: tuple[int, int], corners: np.ndarray, pattern_found: bool
) -> np.ndarray:
    """
    Function that draws the corners of a chessboard pattern on an image.

    Args:
        image (np.ndarray): The input image on which to draw the chessboard corners.
        pattern_size (tuple [int, int]): The size of the chessboard pattern (number of inner corners per row and column).
        corners (np.ndarray): An array of the corner positions to be drawn.
        pattern_found (bool): A boolean indicating whether the corners were found.

    Returns:
        annotated_image (np.ndarray): The output image with the drawn chessboard corners.
    """

    # Draw the chessboard corners using OpenCV implementation
    t_annotated_image = cv2.drawChessboardCorners(image, pattern_size, corners, pattern_found)
...
def load_intrinsics_from_json(
    ...
    """
    Function that loads the intrinsic camera parameters from a JSON file.
    ...
    with open(input_path, "r") as t_file:
        t_intrinsics_dict = json.load(t_file)

    # Extract the intrinsic camera parameters
    t_camera_matrix = np.array(t_intrinsics_dict["camera_matrix"]["data"], dtype=np.float32).reshape(
        t_intrinsics_dict["camera_matrix"]["rows"], t_intrinsics_dict["camera_matrix"]["cols"]
    )
    t_distortion_coefficients = np.array(
        t_intrinsics_dict["distortion_coefficients"]["data"], dtype=np.float32
    ).reshape(
        t_intrinsics_dict["distortion_coefficients"]["rows"],
        t_intrinsics_dict["distortion_coefficients"]["cols"],
    )
    t_image_size = tuple(t_intrinsics_dict["image_size"])
    t_reprojection_error = t_intrinsics_dict["reprojection_error"]

    return t_camera_matrix, t_distortion_coefficients, t_image_size, t_reprojection_error


def load_extrinsics_from_json(
    input_path: str,
) -> tuple[np.ndarray, np.ndarray, float]:
    """
    Function that loads the extrinsic camera parameters from a JSON file.
    ...
    # Load the dictionary from the JSON file
    with open(input_path, "r") as t_file:
        t_extrinsics_dict = json.load(t_file)

    # Extract the extrinsic camera parameters
    t_rotation_matrix = np.array(t_extrinsics_dict["rotation_matrix"]["data"], dtype=np.float32).reshape(
        t_extrinsics_dict["rotation_matrix"]["rows"], t_extrinsics_dict["rotation_matrix"]["cols"]
    )
    t_translation_vector = np.array(t_extrinsics_dict["translation_vector"]["data"], dtype=np.float32).reshape(
        t_extrinsics_dict["translation_vector"]["rows"],
        t_extrinsics_dict["translation_vector"]["cols"],
    )
    t_reprojection_error = t_extrinsics_dict["reprojection_error"]

    return t_rotation_matrix, t_translation_vector, t_reprojection_error
